...
// Using a single Google API key from environment; key rotation removed
    ...
    if (provider === 'exa') {
      ...
      if (!googleApiKey) {
        return NextResponse.json(
          {
            error:
              'Google search API key is not configured. Please set GOOGLE_SEARCH_API_KEY in your environment.',
          },
          { status: 500 }
        )
      }
