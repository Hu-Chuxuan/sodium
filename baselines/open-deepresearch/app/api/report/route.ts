...
import { getTotalTokens, resetTokens } from '@/app/api/costTracker';
import fs from 'fs';
import { COST_DICT } from './cost'
...
{
  "title": "Report title",
  "result" : "The result of consolidated findings. Ensure if the user query is a statement, the output is TRUE or FALSE. Otherwise, the output should be the answer to the user query if it is a question, shortened to a few words",
...
7. It's completely fine if some sources aren't cited at all - this means they weren't needed for the specific analysis requested.`
    ...
    console.log("Here is the given user query: ", query)
    console.log("Here is the given prompt: ", prompt)
    const systemPrompt = generateSystemPrompt(selectedResults, prompt, prompt)
    ...
    try {
      ...
      try {
        ...
        const [inputTokens, outputTokens] = getTotalTokens()
        reportData.inputTokens = inputTokens
        reportData.outputTokens = outputTokens
        reportData.cost = (reportData.inputTokens * COST_DICT[model].cost_per_input_token + reportData.outputTokens * COST_DICT[model].cost_per_output_token);
